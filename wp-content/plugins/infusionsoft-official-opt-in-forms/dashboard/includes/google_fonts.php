...
class INF_Dashboard_Fonts {
	...
	function et_get_google_fonts() {
		...
		return apply_filters( 'inf_google_fonts', $google_fonts );
	}
		...
		foreach ( $et_gf_font_names as $et_gf_font_name ) {
			...
			$query_args = array(
				'family' => sprintf( '%s:%s',
					...
					apply_filters( 'inf_gf_set_styles', $google_fonts[$et_gf_font_name]['styles'], $et_gf_font_name )
				),
