...
class InfusionsoftWordPressSDKException extends Exception {
}

class InfusionsoftWordPressSDK {
	 ...
	 *
	 * @return bool
	 * @throws InfusionsoftWordPressSDKException
	 */
	...
	public function vendorCon( $name, $user, $pass, $key = "", $dbOn = "on" ) {
		...
		if ( $key != "" && $key != "on" && $key != "off" && $key != 'kill' && $key != 'throw' ) {
			...
			if ( ! empty( $details[ $name ] ) ) {
				if ( $details[ $name ][2] == "i" ) {
					...
					throw new InfusionsoftWordPressSDKException( "Invalid application name: \"" . $name . "\"" );
				}
			} else {
				throw new InfusionsoftWordPressSDKException( "Application Does Not Exist: \"" . $name . "\"" );
			}
			$this->key = $details[ $name ][3];
		}
		...
		try {
			...
			throw new InfusionsoftWordPressSDKException( "Connection Failed" );
		}
	...
	public function methodCaller( $service, $callArray ) {
		...
		if ( ! $result->faultCode() ) {
			...
			if ( $this->debug == "kill" ) {
				...
				throw new InfusionsoftWordPressSDKException( $result->faultString(), $result->faultCode() );
	 ...
	 * @description Gets a list of all of the affiliates with their contact data for the specified program.  This
	 *     includes all of the custom fields defined for the contact and affiliate records that are retrieved.
	 *
	 * @param int $programId
	 *
	 * @return array
	 ...
	 * @description Returns a list with each row representing a single affiliates totals represented by a map with key
	 *     (one of the names above, and value being the total for that variable)
	 *
	 * @param array $affList
	 *
	 * @return array
	...
	public function deleteWordPressOptin( $optinId ) {
		$carray = array(
			php_xmlrpc_encode( $optinId )
		);

		return $this->methodCaller( "WordPressService.deleteWordPressOptin", $carray );
	}

	public function syncWordPressOptins( $optins ) {
		$carray = array(
			php_xmlrpc_encode( $optins )
		);

		return $this->methodCaller( "WordPressService.syncWordPressOptins", $carray );
	}
	 ...
	 * @return int
	 */
	public function addOrderTotalDiscount(
		$name,
		$description,
	 ...
	 * @return int
	 */
	public function addProductTotalDiscount(
		$name,
		$description,
		$applyDiscountToCommission,
		$productId,
	...
	public function achieveWordPressGoal( $contactId, $optinId ) {
		$carray = array(
			php_xmlrpc_encode( (int) $contactId ),
			php_xmlrpc_encode( (string) $optinId )
		);
	 ...
	 * @return int
	 */
	public function addRecurringAdv(
		$cid,
		$allowDup,
		$progId,
		$qty,
